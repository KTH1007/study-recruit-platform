...
public interface NotificationControllerDoc {
    ...
    ResponseEntity<ApiResponse<Page<NotificationResponse>>> findNotifications(
            UUID userId, @Parameter(hidden = true) Pageable pageable);

    @Operation(summary = "미읽음 알림 수 조회", description = "읽지 않은 알림 개수를 조회합니다.")
    ResponseEntity<ApiResponse<Long>> countUnread(UUID userId);

    @Operation(summary = "알림 읽음 처리", description = "특정 알림을 읽음 처리합니다.")
    ResponseEntity<ApiResponse<Void>> markAsRead(UUID userId, UUID notificationId);

    @Operation(summary = "전체 알림 읽음 처리", description = "모든 알림을 읽음 처리합니다.")
    ResponseEntity<ApiResponse<Void>> markAllAsRead(UUID userId);
}
